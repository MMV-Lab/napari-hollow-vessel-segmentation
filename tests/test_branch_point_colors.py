...
def test_branch_points_share_one_color() -> None:
    ...
    assert _branch_points_color_for_layer_name("BranchPoints_1") == DEFAULT_BRANCH_POINTS_COLOR
    assert _branch_points_color_for_layer_name("BranchPoints_12") == DEFAULT_BRANCH_POINTS_COLOR
    assert "magenta" not in BRANCH_POINTS_COLOR_CYCLE
    assert _sanitize_branch_display_color("magenta") == DEFAULT_BRANCH_POINTS_COLOR


def test_draft_branch_color_by_layer_name() -> None:
    from regiongrow._widget import (
        BRANCH_POINTS_COLOR_CYCLE,
        DEFAULT_BRANCH_POINTS_COLOR,
        _draft_branch_archive_color,
        _draft_branch_color_for_layer_name,
        _draft_branch_layer_color_index,
    )

    assert _draft_branch_layer_color_index("Draft_Branch") == 0
    assert _draft_branch_layer_color_index("Draft_Branch (1)") == 1
    assert _draft_branch_layer_color_index("Draft_Branch (12)") == 12

    assert _draft_branch_color_for_layer_name("Draft_Branch") == DEFAULT_BRANCH_POINTS_COLOR
